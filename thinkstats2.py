...
def PlotPredictions(daily, years, iters=101, percent=90, func=RunLinearModel):
    ...
    thinkplot.FillBetween(years, low, high, alpha=0.3, color = 'blue')
...
def RunTests(dfLive, piters):
    """Runs the tests from Chapter 9 with a subset of the data.

    dfLive: DataFrame
    piters: how many iterations to run
    """
    n = len(dfLive)
    firsts = dfLive[dfLive.birthord == 1]
    others = dfLive[dfLive.birthord != 1]

    # compare pregnancy lengths
    data = firsts.prglngth.values, others.prglngth.values
    ht = DiffMeansPermute(data)
    p1 = ht.PValue(iters=piters)

    data = (firsts.totalwgt_lb.dropna().values,
            others.totalwgt_lb.dropna().values)
    ht = DiffMeansPermute(data)
    p2 = ht.PValue(iters=piters)

    # test correlation
    dfLive2 = dfLive.dropna(subset=['agepreg', 'totalwgt_lb'])
    data = dfLive2.agepreg.values, dfLive2.totalwgt_lb.values
    ht = CorrelationPermute(data)
    p3 = ht.PValue(iters=piters)

    # compare pregnancy lengths (chi-squared)
    data = firsts.prglngth.values, others.prglngth.values
    ht = PregLengthTest(data)
    p4 = ht.PValue(iters=piters)
...
def RMSE(estimates, actual):
    ...
    Computes the root Mean Squared Error (MSE) of a sequence of estimates.
...
def estimate_var(n, iters):
    """The following function simulates experiments where we try to estimate the variance of a population
       based on a sample with size n. We run iters experiments and two estimates for each sample, s2 and s2n−1.
    ...
    print('RMSE Bias:')
    print('Sample Size {0}, Number of Iterations {1}'.format(n, iters))
    print('biased', RMSE(estimates1, sigma**2))
    print('unbiased', RMSE(estimates2, sigma**2))


def estimate_mean(n, iters):
    """This function simulates experiments where we try to estimate the mean of a population
       based on a sample with size n. We run iters experiments and collect the mean and median of each sample.
    ...
    print('Mean Error:')
    print('Sample Size {0}, Number of Iterations {1}'.format(n, iters))
    print('xbar', RMSE(means, mu))
    print('median', RMSE(medians, mu))


def VertLine(x, y=1):
    thinkplot.Plot([x, x], [0, y], color = '0.8', linewidth = 2)


def sampling_dis(lam, n, iters):
...
def BinnedPercentiles(df, rgIni, rgEnd, pDist):
    """Bin the data by age and plot percentiles of weight for each bin.

    df: DataFrame
    """
    bins = np.arange(rgIni, rgEnd, pDist)
...
def ScatterPlot(ages, weights, alpha = 1.0, s = 20):
    ...
    alpha: float
    """
    thinkplot.Scatter(ages, weights, alpha = alpha, s = s)
...
def EstimateGoals(lam, m):
    """The following function simulates many games,
       then uses the number of goals scored as an estimate of the true long-term goal-scoring rate.
    """
    estimates = []
    for i in range(m):
        L = SimulateGame(lam)
